...
import {
  createTimelineEntry as createInDb,
  findTimelineEntriesByJourney as findManyInDb,
  findTimelineEntryById as findByIdInDb,
  updateTimelineEntry as updateInDb,
  deleteTimelineEntry as deleteInDb,
...
async function verifyEntryOwnership(entryId: string, journeyId: string, userId: string) {
...
export async function getTimelineEntries(journeyId: string, userId: string) {
  await verifyJourneyOwnership(journeyId, userId);
